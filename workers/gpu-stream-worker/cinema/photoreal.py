...
class PhotorealFX:
    ...
    def ensure_cash_rain(self, w: int, count: int = 55) -> None:
        ...
        for _ in range(count):
            self.bills.append(TexturedBill(
                x=float(np.random.uniform(0, w)), y=float(np.random.uniform(-300, -20)),
                vy=float(np.random.uniform(4, 11)), rot=float(np.random.uniform(0, 360)),
                vr=float(np.random.uniform(-5, 5)), scale=float(np.random.uniform(0.65, 1.35)),
            ))
    ...
    def ensure_confetti(self, w: int, count: int = 70) -> None:
        ...
        for _ in range(count):
            self.confetti.append((float(np.random.uniform(0, w)), float(np.random.uniform(-120, 0)),
                float(np.random.uniform(-2.5, 2.5)), float(np.random.uniform(2.5, 8)),
                colors[np.random.randint(0, len(colors))], float(np.random.uniform(5, 12))))
...
CELEBRATION_SCENES = frozenset({"celebration_dance", "fireworks_crown", "dance_finale", "whale_alert", "meme_montage"})


def _dynamic_titles(scene_id, state, symbol):
...
def render_photoreal_frame(scene_id, local_frame, width, height, symbol="KWIF", fx=None, scene_state=None):
    base = _cache.get(scene_id, width, height).copy()
    if scene_id not in _burns:
        _burns[scene_id] = KenBurnsState(image=base, zoom_rate=0.0006 + (hash(scene_id) % 5) * 0.0001,
    ...
    crop = base[y0:y0+zh, x0:x0+zw] if base[y0:y0+zh, x0:x0+zw].size else base
    frame = cv2.resize(crop, (width, height), interpolation=cv2.INTER_LANCZOS4)
    grades = {"lake_intro": (1.05,1.1,1.2), "pump_printer": (1.0,1.15,1.0), "purple_chart": (1.15,0.95,1.2),
              "pink_chat": (1.2,0.9,1.1), "celebration_dance": (1.1,1.05,0.95)}
...
def render_segment_mp4(scene_id, duration_sec, out_path, width=1280, height=720, fps=30, symbol="KWIF"):
    fname = SCENE_IMAGES.get(scene_id, "neon_blue.jpg")
    ...
    vf = (f"scale={width*2}:{height*2},zoompan=z='min(zoom+0.0012,1.4)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':"
          f"d={frames}:s={width}x{height}:fps={fps},drawbox=x=0:y=0:w=iw:h=90:color=black@0.55:t=fill,")
    subprocess.run([os.environ.get("FFMPEG_PATH", "ffmpeg"), "-hide_banner", "-loglevel", "error", "-y",
        "-loop", "1", "-i", str(img_path), "-vf", vf, "-t", str(duration_sec),
        "-c:v", "libx264", "-pix_fmt", "yuv420p", "-crf", "20", str(out_path)], check=True, timeout=300)
    return out_path
